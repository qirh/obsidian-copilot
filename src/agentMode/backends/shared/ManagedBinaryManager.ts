...
export interface ManagedBinaryInstallOptions<TProgress> {
  ...
  /** Keep previously published versions immutable during a background update. */
    ...
    try {
      await this.runExclusive({ kind: "installing", progress: null }, async (signal) => {
        ...
        if (
          ...
          return;
        if (!canStart() || signal.aborted) return;
      ...
      if (attempted)
        notify(`${this.displayName} updated to ${pin}. Existing chats keep their current runtime.`);
    } catch (error) {
      if (!attempted) return;
